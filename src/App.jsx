...
const SkeletonCategoryPage = lazy(() => import('@/components/SkeletonComponents').then(m => ({ default: m.SkeletonCategoryPage })));
const SkeletonStoryPage = lazy(() => import('@/components/SkeletonComponents').then(m => ({ default: m.SkeletonStoryPage })));
const LoadingSpinner = lazy(() => import('@/components/SkeletonComponents').then(m => ({ default: m.LoadingSpinner })));
      ...
      return (
        <div className="min-h-screen bg-white dark:bg-black flex items-center justify-center p-4">
          <div className="text-center">
            ...
            <button onClick={() => window.location.reload()} className="bg-blue-600 text-white px-4 py-2 rounded">Refresh Page</button>
          </div>
        </div>
      );
    }
...
const useNavigationHandler = () => {
  ...
  return useCallback((path) => { try { navigate(path); } catch { window.location.href = path; } }, [navigate]);
};

const sanitizeText = (t) => (t && typeof t === 'string') ? t.replace(/[^\w\s\-.,!?'"]/g, '').trim() : '';
  ...
  useEffect(() => {
    const run = async () => {
      try {
        ...
        const found = allData.all.find((it) => it.id === id || String(it.id) === String(id));
        if (found) {
          setStory(found);
          setRelatedStories(allData.all.filter((it) => it.category === found.category && it.id !== found.id).slice(0, 3));
        } else {
          ...
          <div className="lg:w-2/3">
            <article className="story-card-borderless mb-6 lg:mb-8">
              <div className="mb-4 lg:mb-6">
                <span className="text-xs lg:text-sm font-semibold uppercase tracking-wide px-2 py-1 lg:px-3 lg:py-1 rounded-full text-white" style={{ backgroundColor: 'hsl(var(--orange-accent))' }}>
              ...
              {story.image_url && (
                <div className="story-image-borderless mb-6 lg:mb-8">
                  <img src={story.image_url} alt={cleanTitle(story.title)} className="w-full h-48 lg:h-80 object-cover rounded-xl shadow-md" loading="lazy" onError={(e) => (e.currentTarget.style.display = 'none')} />
                </div>
              )}
              {bulletPoints.length > 0 && (
                <div className="story-summary-borderless">
                  ...
                  <ul className="summary-bullets">
                    {bulletPoints.map((p, i) => (<li key={i}>{sanitizeText(p)}</li>))}
                  </ul>
                </div>
              )}
            </article>
              ...
              <div className="bg-gray-50 dark:bg-gray-800 rounded-xl shadow-lg p-4 lg:p-8 border border-gray-100 dark:border-gray-700">
                ...
                <div className="related-stories-list">
                  {relatedStories.map((rs) => (
                    <div key={rs.id} className="related-story-item">
                      <button onClick={() => safeNavigate(`/article/${rs.id}`)} className="related-story-link w-full text-left">
                        <h3 className="related-story-title-mobile mb-3 font-bold">{cleanTitle(rs.title)}</h3>
                      </button>
                      {/* Use SourceBadge instead of any positivity bar */}
                      <SourceBadge name={(rs.source_name || rs.source || 'Source')} />
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="lg:w-1/3">
            <div className="story-metadata-sidebar">
              ...
              <ul className="metadata-bullets">
                ...
                {Number.isFinite(story.positivity_score) && (<li><strong>Positivity Score:</strong> <span className="ml-2 px-2 py-1 rounded text-xs font-semibold text-white" style={{ backgroundColor: 'hsl(var(--orange-accent))' }}>{Math.round(story.positivity_score)}/10</span></li>)}
              </ul>
              <div className="mt-4 lg:mt-6">
                <a href={story.url} target="_blank" rel="noopener noreferrer" className="block w-full text-center px-4 py-2 lg:px-6 lg:py-3 rounded-lg font-medium transition-all duration-300 hover:shadow-lg text-white text-sm lg:text-base" style={{ backgroundColor: 'hsl(var(--purple-text))' }}>
                  Read Full Article →
                </a>
              </div>
            </div>
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-black">
      ...
      <div className="px-4 lg:px-6 my-6">
        <h2 className="text-xl font-bold mb-6 capitalize">{sanitizeText(decodeURIComponent(category))} News</h2>
        {loading ? (
          <div className="max-w-4xl mx-auto space-y-6"><Suspense fallback={<QuickLoader />}>{Array.from({ length: 4 }, (_, i) => (<QuickLoader key={i} />))}</Suspense></div>
        ) : error ? (
          ...
          <div className="max-w-4xl mx-auto space-y-6">
            {categoryNews.length > 0 ? categoryNews.map((item) => (<NewsCard key={item.id} article={item} />)) : (
              <div className="text-center py-12">
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-black">
      ...
      <div className="main-layout">
        <aside className="daily-reads-sidebar"><div className="daily-reads-separator"><DailyReads stories={dailyReads} /></div></aside>
        <main className="trending-main">
          <h2 className="text-xl font-bold mb-4 text-gray-900 dark:text-white" style={{ color: 'hsl(var(--purple-text))' }}>
            <svg className="inline-block w-6 h-6 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>
            Trending Stories
          </h2>
        ...
        <aside className="blindspot-sidebar"><div className="blindspot-separator"><Blindspot stories={blindspot} /></div></aside>
      </div>
      <Footer />
    </div>
  );
              ...
              <Dialog open={isDonateModalOpen} onOpenChange={setIsDonateModalOpen}>
                <DialogContent>
                  ...
                  <DialogFooter>
                    ...
                    <Button onClick={() => window.open('https://www.patreon.com/c/itsActuallyGoodNews','_blank','noopener,noreferrer')}>Join on Patreon</Button>
                  </DialogFooter>
                </DialogContent>
              </Dialog>
            )}
