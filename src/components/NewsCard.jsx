...
const NewsCard = ({ article, isBookmarked, onBookmarkToggle }) => {
  ...
  const { category, title, summary, source, coverage, image_url, url } = article;
  ...
  const handleShare = (platform) => {
    ...
    switch (platform) {
      case 'instagram':
        urlToOpen = `https://www.instagram.com/?url=${shareUrl}`; // Placeholder – Instagram doesn’t support direct URL shares
        break;
      case 'whatsapp':
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
      <Card className="h-full flex flex-col md:flex-row overflow-hidden bg-card shadow-sm hover:shadow-lg transition-shadow duration-300 border-border/60">
        <div className="md:w-1/3 h-48 md:h-auto">
          <img alt={title} className="h-full w-full object-cover" src={image_url || "https://images.unsplash.com/photo-1657097100900-a218a5178242"} />
        </div>
