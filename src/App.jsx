...
// ✅ Replaced the removed import with a hardcoded list
const allCategories = ['All', 'World', 'Politics', 'Business', 'Tech', 'Science', 'Health', 'Sports', 'Culture', 'Environment'];
...
const HomePage = ({ setIsDonateModalOpen }) => {
  ...
  const getFilteredArticles = () => {
    if (showBookmarks) return articles.filter(a => bookmarkedArticles.includes(a.id));
    if (selectedCategory !== 'All') return articles.filter(a => a.category === selectedCategory);
    return articles;
  };
  ...
  useEffect(() => {
    if (window.location.pathname === '/bookmarks') {
      ...
      const pathCategory = window.location.pathname.startsWith('/category/')
        ? decodeURIComponent(window.location.pathname.split('/category/')[1])
        : 'All';
          ...
          <aside className="hidden lg:block lg:col-span-3 lg:sticky top-24 self-start">
            <div className="space-y-6">
              <div className="bg-card p-6 rounded-lg shadow-sm border">
                ...
                <form className="space-y-3" onSubmit={(e) => { e.preventDefault(); toast({ title: "Feature not implemented" }); }}>
  ...
  return (
    <>
      <Routes>
        ...
        <Route path="/category/:categoryName" element={<HomePage setIsDonateModalOpen={setIsDonateModalOpen} />} />
        <Route path="/bookmarks" element={<HomePage setIsDonateModalOpen={setIsDonateModalOpen} />} />
      </Routes>
